...
server.post("/api/pArr", async function(req, res, next) {
   ...
   // const mapArr = req.body.mar;
	...
	//let userid= "2";
    let values = [update.arr.player, update.arr.map, ];
...
server.put("/api/pArr", async function(req, res, next) {
    ...
    let sql = "UPDATE mapinfo SET playerarr = $1, maparr = $2 WHERE url = $3";

    let values = [update.arr.player, update.arr.map, update.id];
